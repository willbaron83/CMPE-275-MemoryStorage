...
class SpaceBinaryTree:
    ...
    def get_available_space(self, number_of_chunks):
        ...
        '''
        print("[spaceBinavyTree]Number of chunks needed = {}".format(number_of_chunks))
        print("[spaceBinavyTree]resultado del if = {}".format(self.sorted_dict.get(number_of_chunks)))
        if self.sorted_dict.get(number_of_chunks):
            print("[spaceBinavyTree]inside if number of chunks matches available")
            ...
            '''
            try:
                print("[spaceBinavyTree]chunks needed are not found in list, has to be built")
                # Get the largest space available from sorted dict
                largest_chuck = self.sorted_dict.keys()[-1]
                #print("[spaceBinavyTree]largest chunk is = {}".format(largest_chuck))
                ...
                if number_of_chunks < largest_chuck:
                    ...
                    #print("[spaceBinavyTree]Temp node popped out of list = {}".format(temp_node.get_size()))
                    #find if the node free pages list is not empty
                    print("[spaceBinavyTree]Temp node is empty = {}".format(temp_node.is_node_empty()))
                    if not temp_node.is_node_empty():
                        ...
                        print("[spaceBinavyTree]Temp list is = {}".format(len(temp_list)))
